 ...
 * (c) 2010-2014 Google, Inc【谷歌公司】. http://angularjs.org
 * License: MIT
 * 【可以使用，复制和修改软件；
 * 可以免费使用软件或出售；
 ...
 * Angular. It can be called as follows: 【此对象提供一个实用程序，用于在 Angular 
 * 中生成丰富的错误消息。可以称为：】
 *
 ...
 * take. 【上面在 example 命名空间中创建了 minErr 实例。由此产生的错误将有一个命名空间
 * example.one 错误代码。由此产生的错误将用 foo 的值替换 {0} ，用 bar 的值替换 {1} 。
 * 对象可以接受的参数数量不受限制。】
 *
 * If fewer arguments are specified than necessary for interpolation, the extra
 * interpolation markers will be preserved in the final string.【如果指定的参数少
 * 于插值所需的参数，则额外的插值标记将保留在最终字符串中。】
 *
 * Since data will be parsed statically during a build step, some restrictions
 * are applied with respect to how minErr instances are created and called.
 * Instances should have names of the form namespaceMinErr for a minErr created
 * using minErr('namespace') . Error codes, namespaces and template strings
 * should all be static strings, not variables or general expressions.【由于数据
 * 将在构建步骤中进行静态解析，因此会对 minErr 实例的创建和调用方式实施一些限制。实例
 * 应具有使用 minErr('namespace') 创建的 minErr 的格式 namespaceMinErr 的名称。错
 * 误代码、命名空间和模板字符串都应该是静态字符串，而不是变量或通用表达式。】
 *
 * @param {string} module The namespace to use for the new minErr instance.【用于
 * 新miner实例的命名空间】
 * @returns {function(code:string, template:string, ...templateArgs): Error} minErr instance【minErr实例】
 */
...
/* We need to tell jshint what variables are being exported【我们需要告诉jshint要导出哪些变量】 */
/* global angular: true,
    msie: true,
    jqLite: true,
    jQuery: true,
 ...
 * components available within this core module.【ng 模块在启动 AngularJS 应用程序时默认加载。模块本身
 * 包含 AngularJS 应用程序运行所必需的组件。下表列出了此核心模块中可用的每个服务/工厂、筛选器、指令和测试组
 * 件的高级细分。】
 *
 * <div doc-module-components="ng"></div>
 */

// The name of a form control's ValidityState property.【窗体控件的 ValidityState 属性的名称。】
// This is used so that it's possible for internal tests to create mock ValidityStates.【使用它是为了使内部测试能够创建模拟 ValidityStates 。】
 ...
 * @kind function
 *
 * @description Converts the specified string to lowercase.【将指定的字符串转换为小写】
 * @param {string} string String to be converted to lowercase.【要转换为小写的字符串】
 * @returns {string} Lowercased string.【小写字符串】
 */
 ...
 * @kind function
 *
 * @description Converts the specified string to uppercase.【将指定的字符串转换为大写】
 * @param {string} string String to be converted to uppercase.【要转换为大写的字符串】
 * @returns {string} Uppercased string.【大写字符串】
 */
...
// with correct but slower alternatives.【 String#toLowerCase 和 String#toUpperCase 在土耳其语区域设
// 置的浏览器中不会产生正确的结果，因此我们需要检测这种情况，并用正确但较慢的替代方法重新定义小写/大写方法。】
if ('i' !== 'I'.toLowerCase()) {
...
var
    msie,             // holds major version number for IE, or NaN if UA is not IE.【保留 IE 的主要版本号，如果 UA 不是 IE ，则保留 NaN 】
    jqLite,           // delay binding since jQuery could be loaded after us.【延迟绑定，因为 jQuery 可以在我们之后加载】
    jQuery,           // delay binding【延迟绑定】
    slice             = [].slice,
    push              = [].push,
...
/**
 * IE 11 changed the format of the UserAgent string.【 IE 11更改了 UserAgent(用户代理)字符串的格式】
 ...
 *                   String ...)【如果 `obj` 是数组或类似数组的对象（NodeList、Arguments、string...），则返回true】
 */
